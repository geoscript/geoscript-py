...
def processes(text=None):
    ...
    for provider in providers:
        sortedlist = sorted(provider.processes(), key=lambda process: \
                            process.name)
        for process in sortedlist:
            if text == None or text.lower() in process.name.lower() \
                or text.lower() in process.description.lower():
                if len(process.description) < 36:
                    print process.name.ljust(36, '-') + '--->' \
                        + process.description
                else:
