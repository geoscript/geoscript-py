...
from geoscript.processing.parameters import getparameterfromstring, \
    ...
    ParameterTable, ParameterMultipleInput, ParameterBoolean, \
    ParameterSelection
import time
...
def createTempMapset(latlon):
    ...
    outfile.close()
    if latlon:
        outfile = open(os.path.join(folder, 'PERMANENT', 'PROJ_INFO'),
                       'w')
...
def executeGrass(commands):
    if isWindows():
        ...
        os.chmod(grassBatchJobFilename(), stat.S_IEXEC | stat.S_IREAD
                 | stat.S_IWRITE)
        if isMac():
            command = grassPath() + os.sep + 'grass.sh ' \
                + grassMapsetFolder() + '/user'
        else:
...
class GrassProcess(Process):
    ...
    def defineCharacteristicsFromText(self, text):
        ...
        param = ParameterExtent(self.GRASS_REGION_EXTENT_PARAMETER,
                                'Region extent', '0,1,0,1')
        self.inputs[param.name] = param
        param = ParameterNumber(self.GRASS_REGION_CELLSIZE_PARAMETER,
