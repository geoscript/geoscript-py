...
from geoscript.processing.utils import isWindows, userFolder, \
...
def executeSaga():
    if isWindows():
        ...
        os.chmod(sagaBatchJobFilename(), stat.S_IEXEC | stat.S_IREAD
                 | stat.S_IWRITE)
...
class SagaProcess(Process):
    ...
    def defineCharacteristicsFromText(self, text):
        ...
        self.name = 'saga:' + ''.join(c for c in self.fullname if c
                not in ':;,. -_()[]').lower()
    ...
    def _run(self):
        ...
        if isWindows():
            ...
            command = 'lib' + self.undecoratedGroup + ' "' \
                + self.fullname + '"'
        ...
        for out in self.outputs.values():
            if isinstance(out, OutputRaster):
                ...
                if not filename.endswith('.tif'):
                    filename += '.tif'
                    out.value = filename
                filename = tempfolder() + os.sep \
                    + os.path.basename(filename) + '.sgrd'
    ...
    def exportRasterLayer(self, layer):
        ...
        if isWindows():
            ...
            return 'libio_gdal 0 -GRIDS "' + destFilename \
                + '" -FILES "' + layer + '"'
